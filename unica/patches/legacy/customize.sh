...
# The HIDL 2.0 services.jar patch was authored for pre-SDK 36 donors. One UI
# 8.5 changes the relevant smali control flow and synthetic lambda layout, so
# applying that patch to an SDK 36 donor is unsafe and must be skipped.
if [ "$TARGET_PLATFORM_SDK_VERSION" -lt "34" ] && \
        [ "$SOURCE_PLATFORM_SDK_VERSION" -lt "36" ]; then
...
elif [ "$TARGET_PLATFORM_SDK_VERSION" -lt "34" ]; then
    LOGW "Skipping legacy Face HIDL 2.0 services.jar fallback for SDK ${SOURCE_PLATFORM_SDK_VERSION} donor"
fi
