...
ADD_TARGET_VNDK_APEX() {
    case "$TARGET_BOARD_API_LEVEL" in
        "30")
            ...
            ;;
        "31")
            # d2xks needs a fixed v31 APEX because its stock firmware no longer carries
            # one, while other targets retain the upstream b0qxxx prebuilt selection.
            local vndk_v31_source="b0qxxx"
            [[ "$TARGET_CODENAME" == "d2xks" ]] && vndk_v31_source="d2xks"
            ADD_TO_WORK_DIR "$vndk_v31_source" "system_ext" "apex/com.android.vndk.v31.apex" 0 0 644 "u:object_r:system_file:s0"
            ;;
        "32")
