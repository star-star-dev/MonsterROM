...
if ! $SOURCE_WLAN_SUPPORT_MOBILEAP_WIFISHARING_LITE; then
    if $TARGET_WLAN_SUPPORT_MOBILEAP_WIFISHARING_LITE; then
        # Check for target flag instead as we've already took care of this SPF above
        if ! $TARGET_WLAN_SUPPORT_MOBILEAP_POWER_SAVEMODE; then
            wifisharing_lite_patch="$MODPATH/wifi/power_savemode/semwifi-service.jar/0003-Enable-MOBILEAP_WIFISHARING_LITE-support.patch"
        else
            wifisharing_lite_patch="$MODPATH/wifi/wifisharing/semwifi-service.jar/0002-Enable-MOBILEAP_WIFISHARING_LITE-support.patch"
        fi
        if LC_ALL=C git apply --check --directory="$APKTOOL_DIR/system/framework/semwifi-service.jar" \
                --unsafe-paths "$wifisharing_lite_patch" &>/dev/null; then
            APPLY_PATCH "system" "system/framework/semwifi-service.jar" "$wifisharing_lite_patch"
        else
            LOG "- Skipping obsolete MOBILEAP_WIFISHARING_LITE semwifi-service patch"
        fi
        SMALI_PATCH "system" "system/framework/semwifi-service.jar" \
            "smali/com/samsung/android/server/wifi/ap/SemSoftApConfiguration.smali" "replaceall" \
            "SPF_WS_Lite=false" \
